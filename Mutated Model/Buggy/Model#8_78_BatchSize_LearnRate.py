...
current = '{0}/{1}/'.format(currentPATH,'Metrics_78')
...
model.compile(optimizer=keras.optimizers.Adam(learning_rate = 0.5), loss='binary_crossentropy', metrics=['accuracy'])
model.fit(train_X, train_y, batch_size=256, epochs=120, callbacks=[tf.keras.callbacks.Metrics(train_X, train_y, len(model.layers), batch_size=256, lr=0.5, PATH=current)  ])
