...
current = '{0}/{1}/'.format(currentPATH,'Metrics_4')
...
model.compile(optimizer=keras.optimizers.Adam(learning_rate = 0.5), loss='mean_absolute_percentage_error', metrics=['accuracy'])
model.fit(train_X, train_y, batch_size=75, epochs=120, callbacks=[tf.keras.callbacks.Metrics(train_X, train_y, len(model.layers), batch_size=75, lr=0.5, PATH=current)  ])
