...
class mapFaluireToFix(Callback):
    """Callback that terminates training when a NaN loss is encountered.
    """
    def __init__(self, inputs,outputs, layer_number, batch_size, startTime, lr ,epoch, file  ):
...
class Metrics(Callback):
    ...
    def __init__(self, inputs,outputs, layer_number, batch_size, lr, PATH):
    ...
    def on_epoch_end(self, batch, logs=None):
        ...
        batch_x = self.inputs[0:self.batch_size]
        batch_y = self.outputs[0:self.batch_size]
        outputMeanFile = open(self.path+"outputMean.txt", "a")
        outputVarFile = open(self.path+"outputVar.txt", "a")
        outputSTDFile = open(self.path+"outputSTD.txt", "a")
        outputMaxFile = open(self.path+"outputMax.txt", "a")
        outputMinFile = open(self.path+"outputMin.txt", "a")
        outputMedianFile = open(self.path+"outputMedian.txt", "a")
        outputSkewFile = open(self.path+"outputSkew.txt", "a")
        outputSemFile = open(self.path+"outputSem.txt", "a")
        
        MetricFile = open(self.path+"Metric.txt", "a")
        DiffFile = open(self.path+"Metric.txt", "a")
        
        weightMeanFile = open(self.path+"weightMean.txt", "a")
        gradientMeanFile = open(self.path+"gradientMean.txt", "a")
        weightSTDFile = open(self.path+"weightSTD.txt", "a")
        gradientSTDFile = open(self.path+"gradientSTD.txt", "a")
        weightVarFile = open(self.path+"weightVar.txt", "a")
        gradientVarFile = open(self.path+"gradientVar.txt", "a")
        
        weightMaxFile = open(self.path+"weightMax.txt", "a")
        gradientMaxFile = open(self.path+"gradientMax.txt", "a")
        weightMinFile = open(self.path+"weightMin.txt", "a")
        gradientMinFile = open(self.path+"gradientMin.txt", "a")
        
        weightMedianFile = open(self.path+"weightMedian.txt", "a")
        gradientMedianFile = open(self.path+"gradientMedian.txt", "a")
        weightSkewFile = open(self.path+"weightSkew.txt", "a")
        gradientSkewFile = open(self.path+"gradientSkew.txt", "a")
        weightSemFile = open(self.path+"weightSem.txt", "a")
        gradientSemFile = open(self.path+"gradientSem.txt", "a")
    
        VanishingGradientFile = open(self.path+"VanishingGradient.txt", "a")
        DeadNodeFile = open(self.path+"DeadNode.txt", "a")
        LogisticFunFile = open(self.path+"LogisticFun.txt", "a")
        ImproperDataFile = open(self.path+"ImproperData.txt", "a")
        TuneLearnFile = open(self.path+"TuneLearn.txt", "a")
        NormFile = open(self.path+"Norm.txt", "a")
        
        baisesMeanFile = open(self.path+"baisesMean.txt", "a")
        baisesSTDFile = open(self.path+"baisesSTD.txt", "a")
        baisesVarFile = open(self.path+"baisesVar.txt", "a")
        
        

        '***************************************** Training Data  Begin *****************************************'
        ImproperDataFile.write(str(np.min(self.inputs))+"\t"+str(np.max(self.inputs))+"\t")
        ImproperDataFile.write(str(np.min(self.outputs))+"\t"+str(np.max(self.outputs))+"\n")
        ...
        forward_layer = get_layer_output_observation(self.model, batch_x)
        for layerF in range (len(forward_layer)):
            outputMeanFile.write(str(np.mean(forward_layer[layerF]))+"\t")
            outputVarFile.write(str(VarianceResult(forward_layer[layerF]))+"\t")
            outputSTDFile.write(str(stdevResult(forward_layer[layerF]))+"\t")
            outputMaxFile.write(str(np.amax(np.array(forward_layer[layerF]).flatten()))+"\t")
            outputMinFile.write(str(np.amin(np.array(forward_layer[layerF]).flatten()))+"\t")
            outputMedianFile.write(str(np.median(np.array(forward_layer[layerF]).flatten()))+"\t")
            outputSkewFile.write(str(skew(np.array(forward_layer[layerF]).flatten()))+"\t")
            outputSemFile.write(str(stats.sem(np.array(forward_layer[layerF]).flatten(), axis=None, ddof=0))+"\t")
            if (list_name[layerF] == "relu" or list_name[layerF] == "exponential") :
                DeadNodeFile.write(str(DeadResul(forward_layer[layerF], layerF))+"\t")
            if (layerF + 1 < self.lay_num) and (list_name[layerF + 1] =="sigmoid" or list_name[layerF + 1] =="tanh"):
                LogisticFunFile.write(str(SaturatedResult(forward_layer[layerF],list_name[layerF + 1], layerF))+"\t")
        outputMeanFile.write("\n")
        DeadNodeFile.write("\n")
        LogisticFunFile.write("\n")
        outputVarFile.write("\n")
        outputSTDFile.write("\n")
        outputMaxFile.write("\n")
        outputMinFile.write("\n")
        outputMedianFile.write("\n")
        outputSkewFile.write("\n")
        outputSemFile.write("\n")
        '***************************************** End  Begin *****************************************'
        ...
        loss = logs.get('loss')
        if loss is not None:
            MetricFile.write(str(loss)+"\t")
        acc  = logs.get('acc')
        if acc is None:
            acc  = logs.get('accuracy')
        if acc is not None:
            MetricFile.write(str(acc)+"\t")
#        val_loss = logs.get('val_loss')
#        if val_loss is not None:
#            MetricFile.write(str(val_loss)+"\t")
#        
#        val_acc  = logs.get('val_acc')
#        if val_acc is None:
#            val_acc  = logs.get('val_accuracy')
#        if val_acc is not None:
#            MetricFile.write(str(val_acc)+"\t")
        MetricFile.write("\n")
        
      
#        diffLoss = float(loss) - float(val_loss)
#        diffAcc = float(acc) - float(val_acc)
#        DiffFile.write(str(diffLoss)+"\t" + str(diffAcc)+"\t")
#        DiffFile.write("\n")
        
        '***************************************** Metrics  End *****************************************'
        weight_grads = get_weight_grad(self.model,batch_x ,batch_y )
        layer_weights = get_weights_feature(self.model)
        layer_baises = get_baises_feature(self.model)
        boollist =[]
        for layer in self.model.layers:
            if str(layer.name).find('dense') != -1:
                boollist.append('True')
                boollist.append('False')
            if str(layer.name).find('normalization') != -1:
                boollist.append('False')
                boollist.append('False')
        ...
        for layerG in reversed( range (self.lay_num)):                        
            if list_name[layerG].find('dense') != -1:
                if layer_weights[layerG] is not None:
                    weightMeanFile.write(str(np.mean(layer_weights[layerG]))+"\t")
                    weightSTDFile.write(str(stdevResult(layer_weights[layerG]))+"\t")
                    weightVarFile.write(str(VarianceResult(layer_weights[layerG]))+"\t")
                    weightMaxFile.write(str(np.amax(np.array(layer_weights[layerG]).flatten()))+"\t")
                    weightMinFile.write(str(np.amin(np.array(layer_weights[layerG]).flatten()))+"\t")
                    weightMedianFile.write(str(np.median(np.array(layer_weights[layerG]).flatten()))+"\t")
                    weightSkewFile.write(str(skew(np.array(layer_weights[layerG]).flatten()))+"\t")
                    weightSemFile.write(str(stats.sem(np.array(layer_weights[layerG]).flatten()))+"\t")

                    baisesMeanFile.write(str(np.mean(layer_baises[layerG]))+"\t")
                    baisesSTDFile.write(str(stdevResult(layer_baises[layerG]))+"\t")
                    baisesVarFile.write(str(VarianceResult(layer_baises[layerG]))+"\t")
                    weightNorm.append(layer_weights[layerG])
        gradientNorm = []
        for LayerW in reversed( range (len(boollist))):
            if eval(boollist[LayerW]):
                gradientMeanFile.write(str(np.mean(weight_grads[LayerW]))+"\t")
                gradientVarFile.write(str(VarianceResult(weight_grads[LayerW]))+"\t")
                gradientSTDFile.write(str(stdevResult(weight_grads[LayerW]))+"\t")
                gradientMaxFile.write(str(np.amax(np.array(weight_grads[LayerW]).flatten()))+"\t")
                gradientMinFile.write(str(np.amin(np.array(weight_grads[LayerW]).flatten()))+"\t")
                gradientMedianFile.write(str(np.median(np.array(weight_grads[LayerW]).flatten()))+"\t")
                gradientSkewFile.write(str(skew(np.array(weight_grads[LayerW]).flatten()))+"\t")
                gradientSemFile.write(str(stats.sem(np.array(weight_grads[LayerW]).flatten()))+"\t")
                
                VanishingGradientFile.write(str(np.absolute(np.array(weight_grads[LayerW]).flatten()).mean())+"\t")

                gradientNorm.append(weight_grads[LayerW])
                
        gradientMeanFile.write("\n")
        VanishingGradientFile.write("\n")
        weightMeanFile.write("\n")
        baisesMeanFile.write("\n")
        weightVarFile.write("\n")
        baisesVarFile.write("\n")
        gradientVarFile.write("\n")
        weightSTDFile.write("\n")
        baisesSTDFile.write("\n")
        gradientSTDFile.write("\n")
        weightMaxFile.write("\n")
        gradientMaxFile.write("\n")
        weightMinFile.write("\n")
        gradientMinFile.write("\n")
        
        weightMedianFile.write("\n")
        gradientMedianFile.write("\n")
        weightSkewFile.write("\n")
        gradientSkewFile.write("\n")
        weightSemFile.write("\n")
        gradientSemFile.write("\n")
        
    
        for w, dw in zip(weightNorm, gradientNorm):
            if w is not None:
                weight_norm = LA.norm(w.ravel())
                NormFile.write(str(weight_norm)+"\t")
                delta_weight_norm =  LA.norm(dw.ravel())
                NormFile.write(str(delta_weight_norm)+"\t")
                TuneLearnFile.write(str((self.lr *delta_weight_norm )/weight_norm)+"\t")
        ...
        outputMeanFile.close()
        outputVarFile.close()
        outputSTDFile.close()
        MetricFile.close()
        gradientMeanFile.close()
        VanishingGradientFile.close()
        weightMeanFile.close()
        baisesMeanFile.close()
        DeadNodeFile.close()
        LogisticFunFile.close()
        ImproperDataFile.close()
        weightVarFile.close()
        baisesVarFile.close()
        gradientVarFile.close()
        TuneLearnFile.close()
        NormFile.close()
        weightSTDFile.close()
        baisesSTDFile.close()
        gradientSTDFile.close()
        outputMaxFile.close()
        outputMinFile.close()
        outputMedianFile.close()
        outputSkewFile.close()
        outputSemFile.close()
        weightMaxFile.close()
        gradientMaxFile.close()
        weightMinFile.close()
        gradientMinFile.close()
        
        weightMedianFile.close()
        gradientMedianFile.close()
        weightSkewFile.close()
        gradientSkewFile.close()
        weightSemFile.close()
        gradientSemFile.close()
